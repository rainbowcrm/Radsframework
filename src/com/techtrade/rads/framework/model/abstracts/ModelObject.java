...
public abstract class ModelObject {
	...
	private String getJSON(String mode) {
				 ...
				 if (ret == null  ) 
					...
					jsonString.append("\"" + jsonTag + "\":\n");
					 if (radsProp!=null && radsProp.usePKForJSON() ==true )
						 jsonString.append(((ModelObject)ret).toPrimaryKeyJSON());
					 else if (radsProp!=null && radsProp.useBKForJSON() ==true )
						 jsonString.append(((ModelObject)ret).toBusinessKeyJSON());
					 else
						 jsonString.append(((ModelObject)ret).toJSON());
					 jsonString.append(",\n");
				}else if (ret instanceof java.util.Collection) {
					 Collection subObjects = (java.util.Collection)ret ;
					 jsonString.append("\"" + jsonTag + "\":[\n");
					 ...
					 while (it.hasNext()) {
						 ...
						 if (itObject instanceof ModelObject) {
							 if (radsProp!=null && radsProp.usePKForJSON() ==true )
								 jsonString.append(((ModelObject)itObject).toPrimaryKeyJSON());
							 else if (radsProp!=null && radsProp.useBKForJSON() ==true )
								 jsonString.append(((ModelObject)itObject).toBusinessKeyJSON());
							 else
								 jsonString.append(((ModelObject)itObject).toJSON() );
	...
	public String toBusinessKeyJSON()  {
		return getJSON("BK");
	}
	
	public String toPrimaryKeyJSON()  {
		return getJSON("PK");
	}
	...
	private Map<String,Object> getMap(String mode){
				...
				if (ModelObject.class.isAssignableFrom(method.getReturnType()) ) {
					if (retVal == null) {
					 ...
					 if (radsProp!=null && radsProp.usePKForMap() ==true ) {
						 	Map innerMap = ((ModelObject)retVal).getMap("PK");
						 	returnMap.put(key,innerMap);
					 }else if (radsProp!=null && radsProp.useBKForMap() ==true ){
						 	Map innerMap = ((ModelObject)retVal).getMap("BK");
						 	returnMap.put(key,innerMap);
					 }else {
						 	Map innerMap = ((ModelObject)retVal).getMap("full");
						 	returnMap.put(key,innerMap);
					 }
				}else
